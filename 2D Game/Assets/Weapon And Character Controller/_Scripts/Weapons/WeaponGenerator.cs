...
namespace Bardent.Weapons
{
    public class WeaponGenerator : MonoBehaviour
    {
        ...
        private void Update()
        {
            if(weaponCount == 1)
            {
                if (InventoryManager.Instance) Data = InventoryManager.Instance.SwordBn.GetComponent<ItemController>().GetItem()?.weaponData;
            }
            if (weaponCount == 2)
            {
                if (InventoryManager.Instance) Data = InventoryManager.Instance.BowBn.GetComponent<ItemController>().GetItem()?.weaponData;
            }
            if(Data != null)
            {
                GenerateWeapon(Data);
